<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<div class="nova-blog-posts post-display-flex">
			...
			if ( has_post_format( 'quote' ) && NOVA_RWMB_IS_ACTIVE):
				$quote_content = rwmb_meta('bazien_post_quote_content');
				$quote_author = rwmb_meta('bazien_post_quote_author');
				$quote_background = rwmb_meta('bazien_post_quote_bg_color');
				$quote_color = rwmb_meta('bazien_post_quote_text_color');
					...
					if( $excerpt_limit ) {
						if($excerpt_length > 0){
								echo sprintf(
										...
										bazien_excerpt(intval( $excerpt_length ))
								);
						}
					}else {
						echo sprintf(
