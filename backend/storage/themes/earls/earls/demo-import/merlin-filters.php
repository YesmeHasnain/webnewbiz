 ...
 */
function earls_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'earls_unset_default_widgets_args' );
 ...
 */
function earls_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'earls_child_functions_php', 10, 2 );
 ...
 */
function earls_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'earls'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'earls_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'earls' ),
			'preview_url'              => 'https://wp1.themevibrant.com/newwp/earls/',
		),
	);
}

add_filter( 'merlin_import_files', 'earls_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function earls_after_import_setup() {
...
add_action( 'merlin_after_all_import', 'earls_after_import_setup' );