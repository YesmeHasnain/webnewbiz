...
<div class="customer-history order-attribution-metabox">
	<h4>
		<?php
		esc_html_e( 'Total orders', 'nixer' );
		echo wp_kses_post(
			wc_help_tip(
				__( 'Total number of non-cancelled, non-failed orders for this customer, including the current one.', 'nixer' )
			)
		);
		?>
	</h4>
	...
	<h4>
		<?php
		esc_html_e( 'Total revenue', 'nixer' );
		echo wp_kses_post(
			wc_help_tip(
				__( "This is the Customer Lifetime Value, or the total amount you have earned from this customer's orders.", 'nixer' )
			)
		);
		?>
	</h4>
	...
	<h4><?php esc_html_e( 'Average order value', 'nixer' ); ?></h4>
