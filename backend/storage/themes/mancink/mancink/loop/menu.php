...
<nav class="header clean-header not-custom-menu clearfix 
    <?php if (is_singular() && class_exists('Kirki') && get_post_meta(get_the_ID(), 'mancink_default_header', true) != 'head_standard') {
        echo 'white-header';
    } else {
            ...
            <div class="container-fluid">
                <div class="logo-clean">
                    <a href="<?php echo esc_url(home_url('/')); ?>">
                        <?php if (is_singular() && class_exists('Kirki') && get_post_meta(get_the_ID(), 'mancink_default_header', true) != 'head_standard') { ?>
                        <img alt="<?php esc_attr_e('Logo', 'mancink'); ?>" class="logo1"
                            src="<?php if (class_exists('Kirki') && get_theme_mod('mancink_logo_image')) {
                                        echo esc_url(get_theme_mod('mancink_logo_image'));
                                    } else {
                                        echo esc_url(get_template_directory_uri()); ?>/images/logo-white.png <?php } ?>">
                        <?php } else { ?>
                        <img alt="<?php esc_attr_e('Logo', 'mancink'); ?>" class="logo1"
                            src="<?php if (is_singular() && class_exists('Kirki') && get_theme_mod('mancink_logo_image_white')) {
                                        echo esc_url(get_theme_mod('mancink_logo_image_white'));
                                    } else {
                                        echo esc_url(get_template_directory_uri()); ?>/images/logo.png <?php } ?>">
                        <?php  } ?>
                    </a>
                </div>
                <!--/.logo-clean-->
                <div class="box-header hidden-xs hidden-sm">
                    <?php if ( has_nav_menu( 'ridianur-homepage-menu' ) ) { ?>
                    <div class="menu-box">
