...
class ThemeMatcherService
{
    ...
    public function match(string $businessType, string $prompt = ''): string
    {
        ...
        $aiSlug = $this->aiMatch($businessType, $prompt, $layouts);
        if ($aiSlug && isset($layouts[$aiSlug])) {
            Log::info("ThemeMatcher: AI selected '{$aiSlug}' for '{$businessType}'");
            return $aiSlug;
        }

        // Fallback to keyword matching
        $keywordSlug = $this->keywordFallback($businessType, $prompt, $layouts);
        Log::info("ThemeMatcher: keyword fallback selected '{$keywordSlug}' for '{$businessType}'");
        return $keywordSlug;
    }
