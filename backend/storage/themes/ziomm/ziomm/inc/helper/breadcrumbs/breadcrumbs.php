 ...
 * Breadcrumbs
 */
if ( ! function_exists( 'ziomm_get_breadcrumbs' ) ) {
	function ziomm_get_breadcrumbs() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'ziomm' ); // text for the 'Home' link
		$text['category'] = esc_html__( 'Archive by Category "%s"', 'ziomm' ); // text for a category page
		$text['search']   = esc_html__( 'Search Results for "%s"', 'ziomm' ); // text for a search results page
		$text['tag']      = esc_html__( 'Posts Tagged "%s"', 'ziomm' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'ziomm' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'ziomm' ); // text for the 404 page

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'ziomm' ); // text for the 'Home' link
		if( get_option( 'page_for_posts' ) !=='0' ){
		$text['category'] = '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'ziomm' ) . '</a> <span class="sep">-</span> ' . esc_html__( '%s', 'ziomm' ); // text for a category page
		}
		$text['search']   = esc_html__( 'Search Results for "%s"', 'ziomm' ); // text for a search results page
		$text['tag']      = esc_html__( '%s', 'ziomm' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'ziomm' ); // text for an author page
		$text['404']      = esc_html__( '404', 'ziomm' ); // text for the 404 page
		$text['page']     = esc_html__( 'Page %s', 'ziomm' ); // text 'Page N'
		$text['cpage']    = esc_html__( 'Comment Page %s', 'ziomm' ); // text 'Comment Page N'
		...
		if ( is_home() || is_front_page() ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( get_option( 'page_for_posts' ) !=='0' ) {
						$output .= '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'ziomm' ) . '</a> <span class="sep">-</span> '. $cats;
					} else {
						$output .= $cats;
					}
