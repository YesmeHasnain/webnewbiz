<?php
namespace PranaYogaSpace\Admin;

use PranaYogaSpace\Admin\Plugin_Activator\Plugin_Activator;
use PranaYogaSpace\Admin\Upgrader\Upgrader;
...
class Admin {
	...
	public function set_defaults() {
		if ( 'pending' !== get_option( 'cmsmasters_prana-yoga_set_defaults', 'pending' ) ) {
			return;
		}
		...
		update_option( 'cmsmasters_prana-yoga_set_defaults', 'done' );
	}
	...
	private function set_default_data() {
		...
		if ( '' !== $kits && empty( get_option( 'cmsmasters_prana-yoga_default_kits' ) ) ) {
			$kits = json_decode( $kits, true );

			update_option( 'cmsmasters_prana-yoga_default_kits', $kits );
		}

		if ( '' !== $options && empty( get_option( 'cmsmasters_prana-yoga_options' ) ) ) {
			$options = json_decode( $options, true );

			update_option( 'cmsmasters_prana-yoga_options', $options );
		}
	}
	...
	private function set_first_theme_version() {
		if ( get_option( 'cmsmasters_prana-yoga_version', false ) ) {
			return;
		}

		update_option( 'cmsmasters_prana-yoga_version', CMSMASTERS_THEME_VERSION );
	}
		...
		if ( isset( $plugins['cmsmasters-framework/cmsmasters-framework.php'] ) ) {
			...
			echo '<div class="cmsmasters-popup-notice notice notice-error is-dismissible">' .
				'<div class="cmsmasters-popup-notice__inner">
					<p>' .
						sprintf(
							esc_html__( '%s requires CMSMasters Framework to be activate.', 'prana-yoga' ),
							'<strong>' . esc_html__( 'The Theme', 'prana-yoga' ) . '</strong>'
						) . '&nbsp;&nbsp;&nbsp;' .
						...
						) ) . '" class="button button-primary">' . esc_html__( 'Activate', 'prana-yoga' ) . '</a>' .
					'</p>
				</div>
			</div>';
		} else {
					...
					'<div class="cmsmasters-popup-notice__inner">
						<p>' .
							...
							'<a href="' . esc_url( wp_nonce_url(
								...
								'tgmpa-install',
								'tgmpa-nonce'
							) ) . '" class="button button-primary">' . esc_html__( 'Install', 'prana-yoga' ) . '</a>' .
						'</p>
					</div>
				</div>';
				...
				echo '<div class="notice notice-error is-dismissible">
					<p>' .
						sprintf(
							...
							If you need assistance, %4$s', 'prana-yoga' ),
							'<br />',
							'<strong>',
							'</strong>',
							'<a href="https://cmsmasters.studio/help-center/" target="_blank">' . esc_html__( 'please contact our support team', 'prana-yoga' ) . '</a>'
						) .
					'</p>
				</div>';
				...
				echo '<div class="notice notice-error is-dismissible">
					<p>' .
						sprintf(
							...
							You can retry the installation manually in 10 minutes.', 'prana-yoga' ),
							'<br />',
							'<strong>',
							'</strong>',
							'<a href="https://cmsmasters.studio/help-center/" target="_blank">' . esc_html__( 'support team for assistance', 'prana-yoga' ) . '</a>'
						) .
					'</p>
				</div>';
			}
	...
	public function cmsmasters_framework_failed_response_notice() {
		...
		if ( is_numeric( $public_plugins_list ) ) {
			echo '<div class="notice notice-error is-dismissible">
				<p>
					<strong>' . esc_html__( 'GitHub is currently unavailable.', 'prana-yoga' ) . '</strong><br />' .
					esc_html__( 'We were unable to retrieve the plugin list because GitHub was not responding. Please try again in 10 minutes.', 'prana-yoga' ) .
				'</p>
			</div>';
		}
		...
		if ( 'failed_format' === $public_plugins_list ) {
			echo '<div class="notice notice-error is-dismissible">
				<p>
					<strong>' . esc_html__( 'Unexpected plugin data format.', 'prana-yoga' ) . '</strong><br />' .
					sprintf(
						esc_html__( 'The plugin list could not be loaded due to a data format error. Please contact our %s. You can retry the installation manually in 10 minutes.', 'prana-yoga' ),
						'<a href="https://cmsmasters.studio/help-center/" target="_blank">' . esc_html__( 'support team', 'prana-yoga' ) . '</a>'
					) .
				'</p>
			</div>';
		}
