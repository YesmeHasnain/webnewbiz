				...
				<div class="col-lg-12">
					<div class="tp-blog-heading p-relative pb-70">
						<h3 class="tp-breadcrumb-title tp-title-anim">
							<?php if(isset($nixer_redux_demo['search-title']) && $nixer_redux_demo['search-title']!=''){?>
								<?php echo wp_specialchars_decode(esc_attr($nixer_redux_demo['search-title']));?>
							<?php }else{?>
								<?php echo esc_html__( 'Search result for:', 'nixer' );
							}?>
							<span class="text-cap"><?php printf( esc_html__( ' %s', 'nixer' ), get_search_query() );?></span>
						</h3>
					</div>
				</div>
			</div>
			...
			<?php } ?>
					<div class="postbox__wrapper">
						<?php if ( have_posts() ) :  ?>
									...
									<div class="postbox__content">
										...
										<div class="postbox__read-more">
											<a href="<?php the_permalink(); ?>" class="tp-btn-border-lg">
												<?php if(isset($nixer_redux_demo['search-btn-read']) && $nixer_redux_demo['search-btn-read']!=''){?>
													<?php echo wp_specialchars_decode(esc_attr($nixer_redux_demo['search-btn-read']));?>
												<?php }else{?>
							...
							<?php endif; ?>
						<?php else: ?>
							<div class="max-w-1075 mx-auto pb-10 search_custom">
								<h4 class="mb-20">
									<?php if(isset($nixer_redux_demo['search-no-match']) && $nixer_redux_demo['search-no-match']!=''){?>
									<?php echo wp_specialchars_decode(esc_attr($nixer_redux_demo['search-no-match']));?>
									<?php }else{?>
									<?php echo esc_html__( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'nixer' );
									}?>
								</h4>
								<?php get_search_form(); ?>
							</div>
