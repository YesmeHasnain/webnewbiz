...
if ( NOVA_WOOCOMMERCE_IS_ACTIVE ) {
	...
	function woocommerce_template_loop_stock() {
	    ...
	    if ( ! $product->is_in_stock() )
	        echo '<span class="stock out-of-stock">' . esc_html__( 'Out of stock', 'bazien' ) . '</span>';
	}
				...
				<form class="header_search_form" role="search" method="get" action="' . esc_url( home_url( '/'  ) ) .'">
					...
					<div class="header_search_input_wrapper">
						<input
							...
							data-min-chars="3"
							placeholder="' . esc_attr__( 'Product Search', 'bazien' ) . '"
							/>
						...
						if( $categories ) {
							...
							echo '<div class="header_search_select_wrapper">
									<select name="search_category" id="header_search_category_'.$rand_id.'" class="header_search_select">
										<option value="all" selected>' . esc_html__( 'Select Category', 'bazien' ) . '</option>';
	...
	function nova_category_large_icons() {
		if ( !NOVA_WOOCOMMERCE_IS_ACTIVE ) return;
			...
			if ( nova_new_products_page_url() !== false ):
				echo '<a href="' . nova_new_products_page_url() . '"><i class="bazien-icons-ui_star"></i><span>'. nova_new_products_title('') .'</span></a>';
			endif;

			if ( nova_sale_page_url() !== false ):
				echo '<a href="'.nova_sale_page_url().'"><i class="bazien-icons-ecommerce_discount-symbol"></i><span>'. nova_on_sale_products_title('') .'</span></a>';
			endif;
...
if( ! function_exists( 'nova_add_checkout_status_menu' ) ){
	function nova_add_checkout_status_menu() {
		...
		echo '<li>'.esc_html__( '01. Shopping Cart','bazien' ).'</li>';
		echo '<li class="active">'.esc_html__( '02. Shipping & Checkout','bazien' ).'</li>';
		echo '<li>'.esc_html__( '03. Done','bazien' ).'</li>';
		echo '</ul>';
	}
...
if( ! function_exists( 'nova_add_thankyou_status_menu' ) ){
	function nova_add_thankyou_status_menu() {
		echo '<ul class="shopping_cart_status_menu">';
		echo '<li>'.esc_html__( '01. Shopping Cart','bazien' ).'</li>';
		echo '<li>'.esc_html__( '02. Shipping & Checkout','bazien' ).'</li>';
		echo '<li class="active">'.esc_html__( '03. Done','bazien' ).'</li>';
		echo '</ul>';
	}
