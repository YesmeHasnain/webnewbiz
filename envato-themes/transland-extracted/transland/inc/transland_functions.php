 ...
 * Logo
 */
function transland_logo() {

    $opt = get_option('transland_opt');

    $logo = isset($opt['logo_select']) ? $opt['logo_select']: '2';
    
    $main_img_logo = isset($opt['main_logo'] ['url']) ? $opt['main_logo'] ['url'] : TRANSLAND_DIR_IMG.'/logo.svg';
...
// Banner Title
function transland_banner_title() {
    $opt = get_option('transland_opt');
    if( is_home() ) {
        $blog_title = !empty($opt['blog_title']) ? $opt['blog_title'] : esc_html__('News', 'transland');
    ...
    elseif ( is_search() ) {
        esc_html_e( 'Search result for: “', 'transland' ); echo get_search_query().'”';
    }
    elseif ( is_404() ) {
        printf( esc_html__( '404 Error!', 'transland' ) );
    }
    else {
        the_title();
    }
...
// Post's excerpt text
function transland_excerpt($redux_opt, $settings_key, $echo = true) {
...
// Social Links
function transland_social_links() {
    $opt = get_option('transland_opt');
    ?>
...
// transland Social Share Options
function transland_post_share() {
    $opt = get_option('transland_opt');
...
function transland_comments_items($comment, $args, $depth){
     if ( 'ol' === $args['style'] ) {
        $tag       = 'li';
        $add_below = 'comment';
    } else {
                ...
                <div class="info">
                    ...
                    <?php
                        comment_reply_link(
                            array_merge( $args,
                                array( 
                                'reply_text' => '<i class="fal fa-reply"></i>'.esc_html__( 'Reply', 'transland' ),
                                'depth' => $depth,
...
function transland_comment_count($post_id) {
    $comments_number = get_comments_number($post_id);
    if($comments_number == 0) {
        $comment_text = esc_html__('0 comment', 'transland');
    }elseif($comments_number == 1) {
        $comment_text = esc_html__('1 comment', 'transland');
    }elseif($comments_number > 1) {
        $comment_text = $comments_number.esc_html__(' Comments', 'transland');
    }
    echo esc_html($comment_text);
}

// Pagination
function transland_pagination() {
    global $wp_query;
...
add_action('wp_head', 'transland_hook_css');

/**
 * Get a specific html tag from content
 * @return a specific HTML tag from the loaded content
 */
function transland_get_html_tag( $tag = 'blockquote', $content = '' ) {
    $dom = new DOMDocument();
...
add_image_size( 'transland_820x460', 820, 461, true );
add_image_size( 'transland_1170x600', 1170, 658, true );


// Add Span In category number
add_filter('wp_list_categories', 'transland_cat_count_span');

function transland_cat_count_span($links) {
...
function transland_the_archive_count($links) {
    $links = str_replace('</a>'.esc_attr__('&nbsp;','transland').'(', '</a> <span class="number">(', $links);
    $links = str_replace(')', ')</span>', $links);
    return $links;
}

add_filter('get_archives_link', 'transland_the_archive_count');


if ( !function_exists( 'transland_comments_pagination' ) ) {
    function transland_comments_pagination() {
...
function transland_add_upload_mimes($mimes) {
...
add_filter('upload_mimes', 'transland_add_upload_mimes');
