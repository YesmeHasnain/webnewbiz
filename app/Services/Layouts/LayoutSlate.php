...
/* Responsive */
@media(max-width:1100px){.slate-photo{min-height:200px;}}
@media(max-width:700px){.slate-ci{flex-direction:column;align-items:center;text-align:center;}}
</style>
CSS;
    }
...
JS;
    ...
    public function buildHomePage(array $c, array $img): array
    {
        ...
        $heroSub = $c['hero_subtitle'] ?? 'We create thoughtful, refined experiences for brands that value simplicity and substance.';
        $heroCta = $c['hero_cta'] ?? 'View Work';
        $heroCtaUrl = $c['hero_cta_url'] ?? '#portfolio';

        $aboutTitle = $c['about_title'] ?? 'About Us';
        $aboutText = $c['about_text'] ?? 'We are a creative studio focused on delivering clean, purposeful design that speaks for itself.';
        $aboutText2 = $c['about_text2'] ?? 'Every project is an opportunity to create something meaningful, with careful attention to every detail.';

        $services = $c['services'] ?? [
            ['icon' => '◻', 'title' => 'Brand Identity', 'desc' => 'Building cohesive visual identities that communicate your brand essence with clarity and precision.'],
            ['icon' => '△', 'title' => 'Digital Design', 'desc' => 'Crafting elegant digital experiences that balance aesthetics with functionality and usability.'],
            ['icon' => '○', 'title' => 'Art Direction', 'desc' => 'Guiding the creative vision from concept to completion with refined taste and strategic thinking.'],
        ];

        $testimonials = $c['testimonials'] ?? [
            ['quote' => 'Their minimal approach brought clarity to our brand. The result was elegant and perfectly on point.', 'name' => 'Anna M.', 'role' => 'Creative Director', 'initials' => 'AM'],
            ['quote' => 'Clean, thoughtful work that exceeded expectations. They understand the power of restraint.', 'name' => 'David L.', 'role' => 'Architect', 'initials' => 'DL'],
            ['quote' => 'The attention to detail was remarkable. Every element felt intentional and purposeful.', 'name' => 'Sarah K.', 'role' => 'Gallery Owner', 'initials' => 'SK'],
        ];
        ...
        $stats = $c['stats'] ?? [
            ...
            ['number' => '40', 'suffix' => '+', 'label' => 'Awards Won'],
        ];
        ...
        $sections[] = self::container([
            ...
            'padding' => self::pad(160, 80, 120, 80),
            'min_height' => ['size' => 100, 'unit' => 'vh'],
            'background_background' => 'classic',
            'background_color' => $this->colors()['bg'],
            '_element_id' => 'hero',
        ], [
            self::html('<p class="eyebrow" style="opacity:0;animation:fadeUp .7s ease .2s forwards;">' . e($c['hero_eyebrow'] ?? $siteName) . '</p>'),
            ...
            $this->headline($heroTitle, 'h1', array_merge(
                ...
                [
                    ...
                    'custom_css' => 'selector{opacity:0;animation:fadeUp .8s ease .35s forwards;max-width:700px;}',
                ]
            )),

            $this->bodyText($heroSub, [
                'custom_css' => 'selector{opacity:0;animation:fadeUp .8s ease .5s forwards;max-width:520px;}',
            ...
            self::container([
                ...
                'gap' => ['size' => 14, 'unit' => 'px'],
                'content_width' => 'full-width',
                'custom_css' => 'selector{opacity:0;animation:fadeUp .8s ease .65s forwards;}',
            ], [
                $this->ctaButton($heroCta, $heroCtaUrl),
                $this->ghostButton($c['hero_ghost_cta'] ?? 'About Us', '#about'),
            ]),
        ]);
        ...
        if ($heroImg) {
            $sections[] = self::container([
                'content_width' => 'full-width',
                'padding' => self::pad(0, 80),
                'background_background' => 'classic',
                'background_color' => $this->colors()['bg'],
            ], [
                self::image($heroImg, [
                    'custom_css' => 'selector img{width:100%;height:500px;object-fit:cover;display:block;}',
                    'css_classes' => 'sr',
                ]),
            ]);
        }
        ...
        $sections[] = self::twoCol(
            [
                $this->eyebrow($c['about_eyebrow'] ?? 'About'),
                $this->headline($aboutTitle, 'h2', [
                    'typography_font_size' => ['size' => 42, 'unit' => 'px'],
                    '_margin' => self::margin(0, 0, 20, 0),
                ]),
                $this->bodyText($aboutText),
                $this->bodyText($aboutText2, ['_margin' => self::margin(0, 0, 28, 0)]),
                $this->ctaButton($c['about_cta'] ?? 'Learn More', '/about/'),
            ],
            [self::image($aboutImg, [
                'custom_css' => 'selector img{width:100%;height:100%;min-height:400px;object-fit:cover;}',
                'css_classes' => 'sr d2',
            ])],
            50,
            ['padding' => self::pad(120, 80), '_element_id' => 'about', 'background_background' => 'classic', 'background_color' => $this->colors()['surface']],
            ['css_classes' => 'sr', 'flex_justify_content' => 'center'],
            ['padding' => self::pad(0)]
        );
        ...
        $sections[] = $this->section([
            ...
            $this->eyebrow($c['services_eyebrow'] ?? 'Services'),
            $this->headline($c['services_title'] ?? 'What We Do', 'h2'),
            $this->bodyText($c['services_subtitle'] ?? 'Focused expertise across key creative disciplines.', [
                '_margin' => self::margin(0, 0, 0, 0),
        ...
        // ─── PORTFOLIO PREVIEW ───
        $photoEls = [];
        foreach (array_slice($galleryImgs, 0, 3) as $i => $url) {
            if (!$url) continue;
            $photoEls[] = self::container([
                'content_width' => 'full-width',
                'width' => ['size' => 33.33, 'unit' => '%'],
                'css_classes' => 'slate-photo sr d' . ($i + 1),
                'custom_css' => 'selector{min-height:320px;}',
            ], [
                self::image($url, [
                    'custom_css' => 'selector img{width:100%;height:320px;object-fit:cover;}',
                ]),
                self::html('<div class="slate-overlay"><span>View</span></div>'),
            ]);
        }
        ...
        if ($photoEls) {
            $sections[] = $this->section([
                ...
                $this->eyebrow($c['portfolio_eyebrow'] ?? 'Selected Work'),
                $this->headline($c['portfolio_title'] ?? 'Recent Projects', 'h2', [
        ...
        foreach ($testimonials as $i => $t) {
            $testCards[] = self::container([
                'content_width' => 'full-width',
                'flex_direction' => 'column',
                'width' => ['size' => 33.33, 'unit' => '%'],
                'padding' => self::pad(40, 36),
                'background_background' => 'classic',
                'background_color' => $this->colors()['surface'],
                'css_classes' => 'slate-tcard sr d' . ($i + 1),
            ], [
                self::textEditor('<p style="font-size:32px;color:' . $this->colors()['accent'] . ';line-height:1;margin-bottom:16px;">"</p>'),
                self::textEditor('<p style="font-size:15px;color:' . $this->colors()['muted'] . ';line-height:1.8;margin-bottom:24px;">' . e($t['quote']) . '</p>'),
                self::html('<div style="display:flex;align-items:center;gap:12px;"><div style="width:40px;height:40px;background:' . $this->colors()['surface2'] . ';display:flex;align-items:center;justify-content:center;font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:14px;font-weight:600;color:' . $this->colors()['primary'] . ';">' . e($t['initials'] ?? 'AB') . '</div><div><div style="font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:14px;font-weight:600;color:' . $this->colors()['text'] . ';">' . e($t['name']) . '</div><div style="font-size:12px;color:' . $this->colors()['accent'] . ';">' . e($t['role']) . '</div></div></div>'),
            ]);
        }
        ...
        $sections[] = $this->section([
            ...
            $this->headline($c['testimonials_title'] ?? 'Client Words', 'h2', [
                '_margin' => self::margin(0, 0, 50, 0),
            ]),
            self::container([
                'flex_direction' => 'row',
        ...
        $sections[] = self::container([
            ...
            self::html('<p class="eyebrow" style="color:rgba(255,255,255,0.4);">' . e($c['cta_eyebrow'] ?? 'Get in Touch') . '</p>'),
            $this->headline($c['cta_title'] ?? 'Start a Project', 'h2', array_merge(
                self::responsiveSize(56, 42, 32),
                ['title_color' => '#FFFFFF', 'align' => 'center', '_margin' => self::margin(16, 0, 20, 0)]
            )),
            $this->bodyText($c['cta_text'] ?? 'We would love to hear about your next project. Let us create something meaningful together.', [
                'align' => 'center',
                'text_color' => 'rgba(255,255,255,0.55)',
                'custom_css' => 'selector{max-width:480px;margin-left:auto;margin-right:auto;}',
                '_margin' => self::margin(0, 0, 40, 0),
            ]),
        ...
        // Back to top
        $sections[] = self::html('<a href="#hero" id="slate-btt">↑</a>');
...
<button type="submit" style="padding:16px 40px;background:' . $this->colors()['primary'] . ';color:#FFFFFF;font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:13px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;border:none;cursor:pointer;transition:background .3s;">Send Message</button>
    ...
    public function buildHeader(string $siteName, array $pages): array
    {
        ...
        return [self::html('<nav class="slate-nav" id="mainNav" style="position:fixed;top:0;left:0;right:0;z-index:1000;padding:0 80px;height:80px;display:flex;align-items:center;justify-content:space-between;transition:background .4s,box-shadow .4s,height .3s;background:transparent;">
<a href="/" style="font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:18px;font-weight:700;letter-spacing:0.5px;color:' . $this->colors()['text'] . ';text-decoration:none;">' . e($siteName) . '</a>
...
<a href="/contact/" style="padding:10px 24px;background:' . $this->colors()['primary'] . ';color:#FFFFFF;font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;text-decoration:none;transition:background .3s;">Contact</a>
</nav>
<style>
.slate-nav ul a{display:block;padding:8px 18px;font-family:\'' . $this->fonts()['body'] . '\',sans-serif;font-size:12px;font-weight:500;letter-spacing:1px;text-transform:uppercase;color:' . $this->colors()['muted'] . ';text-decoration:none;transition:color .3s;}
.slate-nav ul a:hover{color:' . $this->colors()['text'] . ';}
.slate-nav.bg{background:rgba(255,255,255,0.95)!important;backdrop-filter:blur(20px);height:64px!important;box-shadow:0 1px 0 ' . $this->colors()['border'] . ';}
@media(max-width:1100px){.slate-nav ul{display:none!important;}.slate-nav{padding:0 28px!important;height:64px!important;background:rgba(255,255,255,0.95)!important;backdrop-filter:blur(20px);box-shadow:0 1px 0 ' . $this->colors()['border'] . ';}}
</style>
<script>window.addEventListener("scroll",()=>{document.getElementById("mainNav").classList.toggle("bg",scrollY>50);});</script>')];
    ...
    public function buildFooter(string $siteName, array $pages, array $contact): array
    {
        ...
        $contactHtml = '<div class="slate-ci"><div class="slate-ci-i">✉</div><div><small>Email</small><span>' . e($email) . '</span></div></div>';
        if ($phone) {
            $contactHtml .= '<div class="slate-ci"><div class="slate-ci-i">☎</div><div><small>Phone</small><span>' . e($phone) . '</span></div></div>';
        }
        if ($address) {
            $contactHtml .= '<div class="slate-ci"><div class="slate-ci-i">◎</div><div><small>Location</small><span>' . e($address) . '</span></div></div>';
        }
        ...
        $sections[] = self::container([
            ...
            self::container([
                ...
                'content_width' => 'full-width',
                'gap' => ['size' => 60, 'unit' => 'px'],
                'padding' => self::pad(72, 80, 56, 80),
                'border_border' => 'solid',
                'border_width' => self::pad(0, 0, 1, 0),
                'border_color' => 'rgba(255,255,255,0.08)',
                'custom_css' => 'selector{display:grid;grid-template-columns:1.5fr 1fr 1fr;}',
            ], [
                self::html('<div><div style="font-family:\'' . $this->fonts()['heading'] . '\',sans-serif;font-size:18px;font-weight:700;color:#FFFFFF;margin-bottom:4px;">' . e($siteName) . '</div><p style="font-size:13px;color:rgba(255,255,255,0.4);line-height:1.8;max-width:260px;margin-top:12px;">' . e($contact['footer_text'] ?? 'Creating purposeful design for brands that value simplicity.') . '</p></div>'),
                self::html('<div class="slate-fc"><h4>Pages</h4><ul>' . $navLinks . '</ul></div>'),
                self::html('<div class="slate-fc"><h4>Contact</h4>' . $contactHtml . '</div>'),
            ]),
            self::container([
                'flex_direction' => 'row',
                ...
                'flex_align_items' => 'center',
                'content_width' => 'full-width',
                'padding' => self::pad(18, 80),
            ], [
                self::textEditor('<p style="font-size:12px;color:rgba(255,255,255,0.25);">&copy; ' . date('Y') . ' ' . e($siteName) . '. All rights reserved.</p>'),
                self::textEditor('<a href="#" style="font-size:11px;color:rgba(255,255,255,0.25);text-decoration:none;">Privacy</a> &nbsp;&nbsp; <a href="#" style="font-size:11px;color:rgba(255,255,255,0.25);text-decoration:none;">Terms</a>'),
            ]),
        ]);
