...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'bazien' ),
			'archive'   => esc_html__( 'Archives', 'bazien' ),
			'blog'      => esc_html__( 'Blog', 'bazien' ),
			'search'    => esc_html__( 'Search results for', 'bazien' ),
			'not_found' => esc_html__( 'Not Found', 'bazien' ),
			'author'    => esc_html__( 'Author Archives:', 'bazien' ),
			'day'       => esc_html__( 'Daily Archives:', 'bazien' ),
			'month'     => esc_html__( 'Monthly Archives:', 'bazien' ),
			'year'      => esc_html__( 'Yearly Archives:', 'bazien' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'bazien' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'bazien' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'bazien' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
