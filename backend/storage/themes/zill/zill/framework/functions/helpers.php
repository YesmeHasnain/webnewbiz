...
if ( ! function_exists( 'zill_get_theme_mod' ) ) {

	function zill_get_theme_mod( $key = '', $default = '' ) {
		$mods = get_theme_mods();
		...
		return apply_filters( 'zill/filter/zill_get_theme_mode', $value, $key, $default, $mods );
	}

}

if ( ! function_exists( 'zill_get_option' ) ) {

	function zill_get_option( $key = '', $default = '' ) {
		$theme_options = get_option( 'zill_options', array() );
		...
		return apply_filters( 'zill/filter/get_option', $value, $key, $default, $theme_options );
	}
...
if ( ! function_exists( 'zill_render_variable' ) ) {
	function zill_render_variable( $variable ) {
		return $variable;
	}
}
 ...
 * @return string
 */
if ( !function_exists('zill_render_style_attribute') ){
    function zill_render_style_attribute( $inline_css ) {
        if(!empty($inline_css)){
...
if ( ! function_exists( 'zill_transfer_text_to_format' ) ) {
	function zill_transfer_text_to_format( $content, $autop = false ) {
		if ( $autop ) {
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'zill_comment' ) ) {

	function zill_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
                ...
                <div id="comment-<?php comment_ID(); ?>" class="comment_container">
                    <p><?php esc_html_e( 'Pingback:', 'zill' ); ?>
                        <span><span><?php comment_author_link(); ?></span></span> <?php edit_comment_link( esc_html__( '(Edit)', 'zill' ), '<span class="edit-link">', '</span>' ); ?>
                    </p>
                </div>
            ...
            <div id="comment-<?php comment_ID(); ?>" class="comment-div">
                ...
                <div <?php comment_class( 'comment_container' ); ?>>
					...
					<?php echo get_avatar( $comment, apply_filters( 'zill_comment_avatar_size', 150 ) ); ?>

                    <div class="comment-text">

                        <div class="meta">
                            <div class="woocommerce-review__author"><?php printf( esc_html__( '%s ', 'zill' ), sprintf( '%s', get_comment_author_link() ) ); ?></div>
                            <span class="woocommerce-review__dash">–</span>
                            <span class="woocommerce-review__published-date"><?php comment_date( 'j M Y' ); ?></span>
                        </div>

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="description comment-entry comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'zill' ); ?></p>
                        <?php endif; ?>
                        ...
                        <span class="comment-footer">
                            ...
                            <?php edit_comment_link( __( 'Edit', 'zill' ) ); ?>
                        </span>
                    </div>
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'zill_modify_comment_form_fields' ) ) {

	function zill_modify_comment_form_fields( $fields ) {

		$commenter = wp_get_current_commenter();
		$req       = get_option( 'require_name_email' );

		$fields['author'] = '<div class="comment-form-author"><input type="text" name="author" id="author" value="' . esc_attr( $commenter['comment_author'] ) . '" placeholder="' . esc_attr__( 'Name (required)', 'zill' ) . '" size="22" tabindex="101"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-name" /></div>';

		$fields['email'] = '<div class="comment-form-email"><input type="text" name="email" id="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" placeholder="' . esc_attr__( 'Email', 'zill' ) . '" size="22" tabindex="102"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-email" /></div>';

		$fields['url'] = '<div class="comment-form-url"><input type="text" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="' . esc_attr__( 'Website', 'zill' ) . '" size="22" tabindex="103" class="input-website" /></div>';

		return $fields;

	}

	add_filter( 'comment_form_default_fields', 'zill_modify_comment_form_fields' );
 ...
 * String to boolean
 */
if ( ! function_exists( 'zill_string_to_bool' ) ) {
	function zill_string_to_bool( $string ) {
 ...
 */
if ( ! function_exists( 'zill_get_post_thumbnail_format' ) ) {
	function zill_get_post_thumbnail_format( $thumbnail_size = 'full', $post_link = true ) {
...
if ( ! function_exists( 'zill_entry_meta_item_category_list' ) ) {
	function zill_entry_meta_item_category_list( $before = '', $after = '', $separator = ', ', $parents = '', $post_id = false ) {
		add_filter( 'get_the_terms', 'zill_exclude_demo_term_in_category' );
		$categories_list = get_the_category_list( '{{_}}', $parents, $post_id );
		zill_deactive_filter( 'get_the_terms', 'zill_exclude_demo_term_in_category' );
		if ( $categories_list ) {
			printf(
				'%3$s<span class="screen-reader-text">%1$s </span><span>%2$s</span>%4$s',
				esc_html_x( 'Posted in', 'front-view', 'zill' ),
...
if ( ! function_exists( 'zill_exclude_demo_term_in_category' ) ) {
	function zill_exclude_demo_term_in_category( $term ) {
		return apply_filters( 'zill/post_category_excluded', $term );
	}
}

if ( ! function_exists( 'zill_deactive_filter' ) ) {
	function zill_deactive_filter( $tag, $function_to_remove, $priority = 10 ) {
		return call_user_func( 'remove_filter', $tag, $function_to_remove, $priority );
	}
}

if ( ! function_exists( 'zill_wpml_object_id' ) ) {
	function zill_wpml_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $ulanguage_code = null ) {
...
if ( ! function_exists( 'zill_is_blog' ) ) {
	function zill_is_blog() {
		return ( is_home() || is_tag() || is_category() || is_date() || is_year() || is_month() || is_author() ) ? true : false;
	}
}

if ( ! function_exists( 'zill_get_wishlist_url' ) ) {
	function zill_get_wishlist_url() {
...
if ( ! function_exists( 'zill_get_compare_url' ) ) {
	function zill_get_compare_url() {
		$compare_page_id = zill_get_theme_mod( 'compare_page', 0 );

		return ( ! empty( $compare_page_id ) ? get_the_permalink( $compare_page_id ) : esc_url( home_url( '/compare/' ) ) );
	}
}

if( !function_exists('zill_minimizeCSS') ) {
    function zill_minimizeCSS( $css ){
