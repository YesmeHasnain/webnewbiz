...
<h2 class="<?php echo esc_attr( $heading_class ); ?>">
	<?php
	if ( $email_improvements_enabled ) {
		echo wp_kses_post( __( 'Order summary', 'nixer' ) );
	}
	if ( $sent_to_admin ) {
	...
	$order_number_string = $email_improvements_enabled ? __( 'Order #%s', 'nixer' ) : __( '[Order #%s]', 'nixer' );
...
<div style="margin-bottom: <?php echo esc_html($email_improvements_enabled ? '24px' : '40px'); ?>;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Product', 'nixer' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Quantity', 'nixer' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Price', 'nixer' ); ?></th>
			</tr>
		</thead>
		<?php } ?>
		<tbody>
		...
		<tfoot>
			...
			if ( $item_totals ) {
				...
				foreach ( $item_totals as $total ) {
					...
					?>
					<tr class="order-totals order-totals-<?php echo esc_attr( $total['type'] ?? 'unknown' ); ?><?php echo esc_html( $last_class ); ?>">
			...
			if ( $order->get_customer_note() && ! $email_improvements_enabled ) {
				?>
				<tr>
					<th class="td text-align-left" scope="row" colspan="2"><?php esc_html_e( 'Note:', 'nixer' ); ?></th>
			...
			if ( $order->get_customer_note() && $email_improvements_enabled ) {
				...
				<tr class="order-customer-note">
					<td class="td text-align-left" colspan="3">
						<b><?php esc_html_e( 'Customer note', 'nixer' ); ?></b><br>
