...
define( 'RASHY_THEME_VERSION', '1.2.4' );
...
function rashy_admin_enqueue_styles() {
	...
	wp_enqueue_style( 'font-themify', get_template_directory_uri() . '/css/themify-icons.css', array(), '1.2.4' );

	wp_enqueue_style( 'rashy-webfont', get_template_directory_uri() . '/css/webfonts.css', array(), '1.2.4' );
...
function rashy_enqueue_styles() {
	...
	//load font awesome
	wp_enqueue_style( 'all-awesome', get_template_directory_uri() . '/css/all-awesome.css', array(), '5.11.2' );
    wp_enqueue_style( 'font-icomoon', get_template_directory_uri() . '/css/font-icomoon.css', array(), '1.2.4' );
	// load font themify icon
	wp_enqueue_style( 'font-themify', get_template_directory_uri() . '/css/themify-icons.css', array(), '1.2.4' );

	
	wp_enqueue_style( 'font-eleganticon', get_template_directory_uri() . '/css/eleganticon-style.css', array(), '1.2.4' );
	...
	// magnific-popup
	wp_enqueue_style( 'magnific-popup', get_template_directory_uri() . '/css/magnific-popup.css', array(), '1.2.4' );
	// perfect scrollbar
	...
	wp_enqueue_style( 'rashy-template', get_template_directory_uri() . '/css/template.css', array(), '1.0' );
...
function rashy_enqueue_scripts() {
	...
	// popup
	wp_enqueue_script( 'jquery-magnific-popup', get_template_directory_uri() . '/js/jquery.magnific-popup.min.js', array( 'jquery' ), '1.2.4', true );
	// unviel
	wp_enqueue_script( 'jquery-unveil', get_template_directory_uri() . '/js/jquery.unveil.js', array( 'jquery' ), '1.2.4', true );
	// perfect scrollbar
...
add_action( 'wp_enqueue_scripts', 'rashy_enqueue_price_slider_assets', 20 );

// Force WooCommerce slider assets to be loaded when needed
function rashy_enqueue_price_slider_assets() {
...
add_action( 'save_post_product', 'rashy_clear_price_filter_cache', 3 );
add_action( 'deleted_post', 'rashy_clear_price_filter_cache' );
add_action( 'woocommerce_update_product', 'rashy_clear_price_filter_cache' );

function rashy_clear_price_filter_cache( $post_id = null ) {
    global $wpdb;

    $transients = $wpdb->get_col(
        "SELECT option_name FROM {$wpdb->options}
         WHERE option_name LIKE '_transient_rashy_price_filter_widget_%'"
    );
...
function goal_add_buy_now_button_single(){
    global $product;
    printf( '<button id="adding-button" type="submit" name="buy-now" value="%d" class="buy_now_button button alt">%s</button>', $product->get_ID(), esc_html__( 'Buy Now', 'rashy' ) );
}

add_action( 'woocommerce_after_add_to_cart_button', 'goal_add_buy_now_button_single' );

/*** Handle for click on buy now ***/
function goal_handle_buy_now(){
...
add_action( 'wp_loaded', 'goal_handle_buy_now' );
