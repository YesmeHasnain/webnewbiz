	...
	if ( have_comments() ) :
		...
		<h2 class="comments-title">
			...
			if ( '1' === $tenweb_builder_theme_comment_count ) {
				printf(
					/* translators: 1: title. */
					esc_html__( 'One thought on &ldquo;%1$s&rdquo;', 'flavor-sandstone' ),
					'<span>' . get_the_title() . '</span>'
				);
			} else {
				printf( // WPCS: XSS OK.
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $tenweb_builder_theme_comment_count, 'comments title', 'flavor-sandstone' ) ),
		...
		<ul class="comment-list">
			<?php wp_list_comments(array('callback' => 'sandstone_comment', 'style' => 'ul')); ?>
		</ul><!-- .comment-list -->
		...
		if ( ! comments_open() ) :
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'flavor-sandstone' ); ?></p>
			<?php
		endif;
