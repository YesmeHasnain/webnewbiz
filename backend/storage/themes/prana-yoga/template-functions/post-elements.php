...
use PranaYogaSpace\Core\Utils\Excerpt;
use PranaYogaSpace\Core\Utils\Utils;
use PranaYogaSpace\TemplateFunctions\Post_Media;
...
class Post_Elements {
	...
	public static function get_post_author( $atts = array() ) {
		...
		$out = '<div class="' . esc_attr( $parent_class ) . '-author">' .
			...
			'<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr__( 'Posts by', 'prana-yoga' ) . ' ' . esc_attr( get_the_author_meta( 'display_name' ) ) . '" class="vcard author">' .
	...
	public static function get_post_comments( $atts = array() ) {
		...
		if ( 'icon' === $type ) {
			$out .= '<a class="cmsmasters-theme-icon-comment cmsmasters-' . esc_attr( $icon_position ) . '" href="' . esc_url( get_comments_link() ) . '" title="' . esc_attr__( 'Comment on', 'prana-yoga' ) . ' ' . esc_attr( get_the_title() ) . '">' .
				'<span>' . esc_html( get_comments_number() ) . '</span>' .
			'</a>';
		} else {
			$out .= esc_html__( 'Comments', 'prana-yoga' ) . ' (' .
			'<a href="' . esc_url( get_comments_link() ) . '" title="' . esc_attr__( 'Comment on', 'prana-yoga' ) . ' ' . esc_attr( get_the_title() ) . '">' .
		...
		foreach ( $elements as $element ) {
			if ( 'categories' === $element ) {
				...
				$out .= self::get_post_terms( array(
					'parent_class' => $parent_class,
					'hierarchical' => false,
					'before' => apply_filters( 'cmsmasters_theme_get_post_meta_tags_before', '<span>' . esc_html__( 'In', 'prana-yoga' ) . ': </span>' ),
