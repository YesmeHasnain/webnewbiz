...
<body id="body" <?php body_class('tp-magic-cursor'); ?>>
	...
	<?php if (!empty($nixer_redux_demo['h6-right-item']) && !empty($nixer_redux_demo['h6-mini-cart-switch'])): ?>
		<?php get_template_part( 'header/header-templates/header', 'minicart' ); ?>
	<?php endif ?>
	<?php get_template_part( 'header/header-templates/header', 'canvas' ); ?>

	<header class="tp-header-6-ptb tp-header-transparent">
		<div id="header-sticky" class="tp-main-sticky p-relative">
			<div class="container container-1720">
				<div class="d-flex justify-content-between align-items-center">
					<div class="tp-header-6-menu inner-white d-none d-xl-block">
					...
					<div class="tp-header-6-logo">
						<a href="<?php echo esc_url(home_url('/')); ?>">
							<?php if (!empty($nixer_redux_demo['h6-logo']['url'])) { ?>
								<img data-width="120" src="<?php echo esc_url($nixer_redux_demo['h6-logo']['url']); ?>" alt="<?php bloginfo( 'name' ); ?>">
							<?php } else { ?>
								<img data-width="120" src="<?php echo esc_url(get_template_directory_uri());?>/assets/img/logo/logo.png" alt="<?php bloginfo( 'name' ); ?>">
							<?php } ?>
						</a>
					</div>
					<?php if (!empty($nixer_redux_demo['h6-right-item'])) { ?>
						<div class="tp-header-6-box d-flex align-items-center">
							<?php if (!empty($nixer_redux_demo['h6-mini-cart-switch'])): ?>
							...
							<?php endif ?>
							<?php if (!empty($nixer_redux_demo['h6-button-switch-1']) || !empty($nixer_redux_demo['h6-button-switch-2'])): ?>
								<div class="tp-header-6-btn-box d-none d-xl-block">
									<?php if (!empty($nixer_redux_demo['h6-link-button-1']) && !empty($nixer_redux_demo['h6-text-button-1'])): ?>	
										<a href="<?php echo esc_attr($nixer_redux_demo['h6-link-button-1']); ?>">
											<?php echo esc_attr($nixer_redux_demo['h6-text-button-1']); ?>
										</a>
									<?php endif ?>
									<?php if (!empty($nixer_redux_demo['h6-link-button-2']) && !empty($nixer_redux_demo['h6-text-button-2'])): ?>
										<a href="<?php echo esc_attr($nixer_redux_demo['h6-link-button-2']); ?>">
											<?php echo esc_attr($nixer_redux_demo['h6-text-button-2']); ?>
										</a>
									<?php endif ?>
								</div>
							<?php endif ?>
					...
					<?php } else { ?>
						<div class="tp-header-6-box d-flex align-items-center d-block d-xl-none">
							<div class="tp-header-hamburger offcanvas-open-btn d-block d-xl-none">
								<button class="tp-hamburger-btn">
									<span></span>
									<span></span>
