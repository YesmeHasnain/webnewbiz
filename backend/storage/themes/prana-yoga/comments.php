...
if ( post_password_required() ) {
	echo '<p class="cmsmasters-nocomments nocomments">' . esc_html__( 'This post is password protected. Enter the password to view comments.', 'prana-yoga' ) . '</p>';
...
if ( sizeof( $pings ) > 0 ) {
	echo '<div class="cmsmasters-pings">' .
		'<h4>' . esc_html__( 'Trackbacks and Pingbacks', 'prana-yoga' ) . '</h4>' .
...
if ( have_comments() ) {
	$comments_nav = get_the_comments_navigation( array(
		'prev_text' => '<span class="' . esc_attr( $parent_class ) . '__nav-arrow cmsmasters-theme-icon-comments-nav-prev"></span><span class="' . esc_attr( $parent_class ) . '__nav-text">' . esc_attr__( 'Older Comments', 'prana-yoga' ) . '</span>',
		'next_text' => '<span class="' . esc_attr( $parent_class ) . '__nav-text">' . esc_attr__( 'Newer Comments', 'prana-yoga' ) . '</span><span class="' . esc_attr( $parent_class ) . '__nav-arrow cmsmasters-theme-icon-comments-nav-next"></span>',
	) );
...
if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
	echo '<h5 class="no-comments ' . esc_attr( $parent_class ) . '_closed">' . esc_html__( 'Comments are closed.', 'prana-yoga' ) . '</h5>';
}

$form_fields = array(
	'author' => '<p class="comment-form-author">' .
		'<input type="text" id="author" name="author" value="' . esc_attr( $commenter['comment_author'] ) . '" size="35"' . ( ( isset( $aria_req ) ) ? $aria_req : '' ) . ' placeholder="' . esc_attr__( 'Your name', 'prana-yoga' ) . ( ( $req ) ? ' *' : '' ) . '" />' .
	'</p>',
	'email' => '<p class="comment-form-email">' .
		'<input type="text" id="email" name="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="35"' . ( ( isset( $aria_req ) ) ? $aria_req : '' ) . ' placeholder="' . esc_attr__( 'Your email', 'prana-yoga' ) . ( ( $req ) ? ' *' : '' ) . '" />' .
	'</p>',
);
...
if ( '1' === get_option( 'show_comments_cookies_opt_in' ) ) {
	$form_fields['cookies'] = '<p class="comment-form-cookies-consent">' .
		...
		'<label for="wp-comment-cookies-consent">' . esc_html__( 'Save my name, email, and website in this browser for the next time I comment.', 'prana-yoga' ) . '</label>' .
	'</p>';
}
...
comment_form( array(
	...
	'comment_field' => '<p class="comment-form-comment">' .
		'<textarea name="comment" id="comment" cols="67" rows="5" placeholder="' . esc_attr__( 'Comment', 'prana-yoga' ) . '"></textarea>' .
	'</p>',
) );
...
function cmsmasters_single_comment_callback( $comment, $args, $depth ) {
	...
	if ( '0' === $comment->comment_approved ) {
		echo '<p>' .
			'<em>' . esc_html__( 'Your comment is awaiting moderation.', 'prana-yoga' ) . '</em>' .
		'</p>';
	}
	...
	'<div class="' . esc_attr( $parent_class ) . '__reply">' .
		get_comment_reply_link( array_merge( $args, array(
			...
			'reply_text' => esc_attr__( 'Reply', 'prana-yoga' ),
		) ) );

		edit_comment_link( esc_attr__( 'Edit', 'prana-yoga' ), '', '' );
