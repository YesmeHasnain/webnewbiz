...
<footer class="footer">
    ...
    <div class="container-fluid">
        ...
        <?php if (class_exists('Kirki') && get_theme_mod('mancink_footer_logo')) { ?>
        <img class="footer-img" src="<?php echo esc_url(get_theme_mod('mancink_footer_logo')); ?>"
            alt="<?php esc_attr_e('Logo', 'mancink'); ?>">
        <?php }  ?>

        <?php if (class_exists('Kirki') && get_theme_mod('mancink_footer_text')) {
            $mancink_fot_text = get_theme_mod('mancink_footer_text');
            echo '<div class="spacing30 clearfix"></div>';
            echo wp_kses_post($mancink_fot_text);
            echo '<div class="spacing30 clearfix"></div>';
        } ?>

        <ul class="footer-icon  <?php if (class_exists('Kirki') && get_theme_mod('mancink_footer_text') == '') {
                                    echo 'with-margin';
                                } ?>">
