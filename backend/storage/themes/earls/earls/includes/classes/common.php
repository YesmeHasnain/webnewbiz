...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => earls_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => earls_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => earls_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => earls_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => earls_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => earls_meta( 'banner_elementor_template', $page_id ),
				'banner'            => earls_set( earls_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => earls_meta( 'banner_banner_title', $page_id ),
				'text'             => earls_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => earls_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : earls_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : earls_the_title( $template ),
				'banner'        => earls_set( $bg, 'url' ),
