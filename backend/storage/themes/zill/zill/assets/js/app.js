...
(function ($) {
    ...
    LaStudio.core.CustomFunction = function () {
        ...
        try {
            if (Cookies.get('zill_wishlist') !== 'undefined') {
                const _cW = JSON.parse(Cookies.get('zill_wishlist'));
                wishlist_compare_count.wishlist = _cW[0].posts.length
            }
            if (Cookies.get('zill_compare') !== 'undefined') {
                const _cC = JSON.parse(Cookies.get('zill_compare'));
        ...
        return {
            init: function () {
                if (waitfortrigger != 1) {
                    ...
                    try {
                        if (Cookies.get('zill_dont_display_popup') == 'yes') {
                            return;
                        }
                    } catch (ex) {
                        ...
                        setTimeout(function () {
                            LaStudio.core.OpenNewsletterPopup($popup, function () {
                                if ($('.cbo-dont-show-popup', $popup).length && $('.cbo-dont-show-popup', $popup).is(':checked')) {
                                    try {
                                        Cookies.set('zill_dont_display_popup', 'yes', {
                                            expires: backtime,
                                            path: '/'
                                        });
                                    } catch (ex) {
