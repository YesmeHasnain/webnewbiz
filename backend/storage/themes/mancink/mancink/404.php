<?php
get_header();

//custom header
if (class_exists('Kirki')) {
	do_action('mancink-custom-header', 'mancink_header_start');
} else { ?>

<!--Fall back if no kirki plugin installed-->
<!--HOME START-->
<div class="default-header clearfix">
    <!--HEADER START-->
    <?php get_template_part('loop/menu', 'normal'); ?>
    <!--HEADER END-->
</div>
<!--/home end-->
<!--HOME END-->

<?php } ?>

<!--breadcrumb if available-->
<?php if (function_exists('mancink_breadcrumb')) { ?>
<div class="box-crumb clearfix">
    <div class="slider-mask"></div>
    <div class="container-fluid">


        <?php if (function_exists('mancink_breadcrumb')) {
				mancink_breadcrumb();
			} ?>
        <h1 class="blog-title"><?php esc_html_e('Page not found', 'mancink'); ?></h1>


    </div>

</div>
<?php } ?>

<div class="clearfix content page-content-wrapper">
    <div class="container-fluid error-container">
        <div class="clearfix error-col">
            <span class="error-title"><?php esc_html_e('404', 'mancink'); ?></span>
            <h2 class="error-bigtext"><?php esc_html_e('Page not found!', 'mancink') ?></h2>
            <p><?php esc_html_e('We could not find that page. If you entered a web address please check it was correct.', 'mancink') ?>
            </p>
            <a class="content-btn" href="<?php echo esc_url(home_url('/')); ?>">
                <?php echo esc_html__('Back to Homepage', 'mancink') ?>
            </a>
        </div>

    </div>
    <!--/.container-->
</div>
<!--/.content-->


<?php //custom footer
//custom footer
if (class_exists('Kirki')) {
	do_action('mancink-custom-footer', 'mancink_footer_start');
} else {
	//fallback if no kirki installed
	get_template_part('loop/bottom', 'footer');
}

get_footer(); ?>