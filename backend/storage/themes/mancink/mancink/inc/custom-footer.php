...
function mancink_footer_start()
{
	if (is_singular()) { //if single page/post
		global $post;
		if (get_post_meta(get_the_ID(), 'mancink_footer_option', true) == 'custom' && get_post_meta(get_the_ID(), 'mancink_meta_choose_footer', true)) {
			$footer_id =  get_post_meta(get_the_ID(), 'mancink_meta_choose_footer', true);

			$mancink_footer = new WP_Query(array(
				'posts_per_page'   => 1,
				'post_type' =>  'footer',
				'p'         => $footer_id,
			));

			if ($mancink_footer->have_posts()) : while ($mancink_footer->have_posts()) : $mancink_footer->the_post();
					global $post; ?>
		...
		//if page setting choose footer global
		else if (get_post_meta(get_the_ID(), 'mancink_footer_option', true) == 'global') {
			...
			//if no footer
		} else if (get_post_meta(get_the_ID(), 'mancink_footer_option', true) == 'none') {
			//do nothing
		} else {
