...
if ( $downloads ) : ?>
	...
	<h2><?php echo apply_filters( 'woocommerce_my_account_my_downloads_title', esc_html__( 'Available downloads', 'nixer' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h2>
		...
		<?php foreach ( $downloads as $download ) : ?>
			<li>
				...
				if ( is_numeric( $download['downloads_remaining'] ) ) {
				    $count = $download['downloads_remaining'];
				    $text = $count == 1 ? __( '1 download remaining', 'nixer' ) : sprintf( __( '%s downloads remaining', 'nixer' ), $count );
				    echo apply_filters(
				        'woocommerce_available_download_count',
				        '<span class="woocommerce-Count count">' . $text . '</span> ',
				        $download
